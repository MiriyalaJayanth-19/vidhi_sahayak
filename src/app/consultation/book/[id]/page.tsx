...
export default function BookConsultationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10">
      ...
      <p className="mt-2 text-sm text-zinc-600">Schedule a session with {lawyer.name} ({lawyer.location}).</p>

      {success && (
        <div className="mt-4 rounded-md border border-green-600/30 bg-green-50 p-3 text-sm text-green-700">
          {success}
        </div>
      )}
      ...
      <form onSubmit={onConfirm} className="mt-6 grid gap-3 rounded-lg border bg-white p-4">
        <div className="grid gap-3 md:grid-cols-2">
          <div>
            <label className="block text-xs font-medium">Name</label>
            <input required name="name" className="mt-1 w-full rounded-md border bg-transparent px-3 py-2 text-sm outline-none" placeholder="Your full name" />
          </div>
          <div>
            <label className="block text-xs font-medium">Email</label>
            <input required name="email" type="email" className="mt-1 w-full rounded-md border bg-transparent px-3 py-2 text-sm outline-none" placeholder="you@example.com" />
          </div>
        </div>
        <div className="grid gap-3 md:grid-cols-2">
          <div>
            <label className="block text-xs font-medium">Date</label>
            <input required name="date" type="date" className="mt-1 w-full rounded-md border bg-transparent px-3 py-2 text-sm outline-none" />
          </div>
          <div>
            <label className="block text-xs font-medium">Time</label>
            <select required name="time" className="mt-1 w-full rounded-md border bg-transparent px-3 py-2 text-sm outline-none">
        ...
        <div>
          ...
          <textarea required name="summary" rows={4} className="mt-1 w-full rounded-md border bg-transparent px-3 py-2 text-sm outline-none" placeholder="Briefly describe your issue" />
        </div>
        <div className="flex items-center gap-2">
          <button type="submit" disabled={submitting} className="rounded-md bg-black px-4 py-2 text-sm font-medium text-white hover:bg-zinc-800 disabled:opacity-60">
