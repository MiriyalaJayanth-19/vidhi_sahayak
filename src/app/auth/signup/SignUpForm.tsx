...
export default function SignUpForm() {
  ...
  async function onSubmitUser(e: React.FormEvent<HTMLFormElement>) {
    ...
    const { error: profileErr } = await sb
      .from("profiles")
      .insert({ id: userId, role: "user", full_name: fullName, email, preferred_language: lang });
  ...
  async function onSubmitLawyer(e: React.FormEvent<HTMLFormElement>) {
    ...
    const { error: profileErr } = await sb
      .from("profiles")
      .insert({ id: userId, role: "lawyer", full_name: fullName, email });
    if (profileErr && profileErr.code !== "23505") {
      console.warn("Profile insert error:", profileErr.message);
    }
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10">
      ...
      <p className="mt-6 text-xs text-zinc-500">
        Note: Authentication and storage will be connected in Phase 2 using Supabase Auth and Storage.
      </p>
    </div>
  );
}
